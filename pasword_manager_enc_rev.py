...
#Check is stored in pc
fileKey = open('key.txt', 'a+')
fileSize=os.path.getsize("key.txt")
#generate key if key is not found
if( fileSize==0):
    key = Fernet.generate_key()
    fileKey.write(str(key))
else:
    for lineKey in open('key.txt'):
        key=str(lineKey)
        ...
        cursor.execute("INSERT INTO manager VALUES (:app_name, :url, :email_id, :password)",
            {
                ...
                'password': fernet.encrypt(password.get().encode())
            }
        )
        # Commit changes
        conn.commit()
...
def query():
    ...
    for record in records:
            p_records += str(record[4])+ " " +str(record[0])+ " " + str(record[1])+ " " + str(record[2]) + " " + str(fernet.decrypt(record[3]).decode())+ "\n"
        #print(record)
